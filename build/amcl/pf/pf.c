...
#define MAP_WXGX(i) (((i) - MAP_SIZE / 2) * MAP_SCALE)
#define MAP_WYGY(j) (((j) - MAP_SIZE/ 2) * MAP_SCALE)
...
void pf_init(pf_t *pf, pf_vector_t mean, pf_matrix_t cov)
{
  ...
  for (i = 0; i < set->sample_count; i++)
  {
    ...
    //printf("sample->pose: %f %f %f\n", MAP_GXWX(sample->pose.v[0]), MAP_GYWY(sample->pose.v[1]), sample->pose.v[2]);
...
void pf_update_sensor(pf_t *pf, pf_sensor_model_fn_t sensor_fn, void *sensor_data)
{
  ...
  pf_sample_t *sample;
  double total;
  //printf("pf_update_sensor\n");
  set = pf->sets + pf->current_set;
  //printf("set->sample_count: %d %d\n", set->sample_count,pf->current_set);
...
void copy_set(pf_sample_set_t* set_a, pf_sample_set_t* set_b)
{
  ...
  for(i = 0; i < set_a->sample_count; i++)
  {
    ...
    if(sample_a->weight <= 0){
      sample_a->weight = 0.00001;
      printf("sample_a->weight: %f\n", sample_a->weight);
    }
