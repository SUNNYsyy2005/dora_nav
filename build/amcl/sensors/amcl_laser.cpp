...
double AMCLLaser::LikelihoodFieldModel(AMCLLaserData *data, pf_sample_set_t* set)
{
  ...
  for (j = 0; j < set->sample_count; j++)
  {
    ...
    if(step < 1)
      step = 1;
    int mmi = MAP_GXWX(self->map, pose.v[0]);
    int mmj = MAP_GYWY(self->map, pose.v[1]);
      //printf("mi:%d %d \n", mi, mj);
      // Part 1: Get distance from the hit to closest obstacle.
      // Off-map penalized as max distance
    bool flag = false;
    if(!MAP_VALID(self->map, mmi, mmj)){
      flag = true;
      tot++;
    }
    else{
      if(self->map->cells[MAP_INDEX(self->map,mmi,mmj)].occ_state==0||self->map->cells[MAP_INDEX(self->map,mmi,mmj)].occ_state==1){
        flag = true;
      }
      tot++;
    } 
