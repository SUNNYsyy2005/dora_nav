  ...
  for(int i = 0; i < map->size_x; i++)
{
    ...
    {
        // 获取当前单元格的occ_dist值
        double occ_dist = map->cells[MAP_INDEX(map, i, j)].occ_dist;
        
        // 检查occ_dist是否在0和max_occ_dist之间
        if(occ_dist > 0.0 && occ_dist < max_occ_dist)
        {
            // 输出单元格坐标
            //printf("Cell at (%d, %d) has occ_dist: %f\n", i, j, occ_dist);
        }
    }
}
  delete[] marked;
